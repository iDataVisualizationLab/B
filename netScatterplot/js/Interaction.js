class Interaction {
   constructor() {
        ...
        plotData.forEach((e,i)=>{
            if (e.data.length > 0) {
                ...
                for (let m in filters) {
                    ...
                    if (m === 'outlying_length' || m === 'outlying_angle') check = check && e.metrics[m].score >= lowerLimit && e.metrics[m].score <= upperLimit;
    ...
    static Sorting(plotData,filteredData,chosenMetric) {
       let array = [];
       if (chosenMetric === 'outlying_length' || chosenMetric === 'outlying_angle') array = filteredData.map(e=>[e,plotData[e].metrics[chosenMetric].score]);
