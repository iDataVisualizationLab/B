...
function outlyingLength (vectors) {
    ...
    let li = q3 + 1.5*(q3-q1);
    let index = dArr.findIndex(e=>e>li);
    let s = 0, so = 0;
    for (let i = 0; i < dArr.length; i++) {
        s += Math.abs(dArr[i]-q2);
        if (i >= index) so += Math.abs(dArr[i]-q2);
    }
    return (s) ? so/s : 0;
}
...
function outlyingAngle (vectors) {
    ...
    let uIndex = aArr.findIndex(e=>e>uli);
    let lIndex = aArr.findIndex(e=>e<lli);
    let s = 0, so = 0;
    for (let i = 0; i < aArr.length; i++) {
        s += Math.abs(aArr[i]-q2);
        if (i <= lIndex || i >= uIndex) so += Math.abs(aArr[i]-q2);
    }
    return s ? so/s : 0;
}
...
function translation (vectors) {
    ...
    if (index !== -1) {
        for (let i = index; i < MST.length; i++) {
            ...
            let check1 = false;
            for (let j = 0; j < MST.length; j++) {
                check0 = MST[i][0] === MST[j][0] || MST[i][0] === MST[j][1];
                check1 = MST[i][1] === MST[j][0] || MST[i][1] === MST[j][1];
                if (check0 && check1) break;
            }
            if (!check0 || !check1) {
                if (!check0) ol.push(MST[i][0]);
                if (!check1) ol.push(MST[i][1]);
            }
        }
        let r = 0;
        ...
        let d = Math.sqrt((c1[0]-c2[0])*(c1[0]-c2[0])+(c1[1]-c2[1])*(c1[1]-c2[1]));
        return r>0 ? d/r : 0;
    } else return 0;
}
...
function homogeneous (vectors) {
    ...
    return 1-Math.sqrt(s/vectors.length);
}